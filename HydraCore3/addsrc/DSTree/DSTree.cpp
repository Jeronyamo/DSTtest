...
void DSTree::qsortUpper(unsigned* inst_arr, unsigned count, unsigned current_scene_axis) {
	...
	while (top > 0) {
		...
		for (unsigned j = 0u; j < last; ++j) {
			...
			if (!res) res = int(&(temp_inst[j]) > &(temp_inst[last])) - int(&(temp_inst[j]) < &(temp_inst[last]));
			if (res < 0) {
				++i;
...
void DSTree::qsortLower(unsigned *inst_arr, unsigned count, unsigned current_scene_axis, LiteMath::float4* tempVertices, unsigned* tempIndices) {
	...
	while (top > 0) {
		...
		int i = -1;
		for (unsigned j = 0u; j < last; ++j) {
			unsigned tempInd = 3 * temp_inst[j];
			float max1 = max(tempVertices[tempIndices[tempInd]][current_scene_axis],
							 max(tempVertices[tempIndices[tempInd + 1u]][current_scene_axis],
								 tempVertices[tempIndices[tempInd + 2u]][current_scene_axis]));

			tempInd = 3 * temp_inst[last];
			float max2 = max(tempVertices[tempIndices[tempInd]][current_scene_axis],
							 max(tempVertices[tempIndices[tempInd + 1u]][current_scene_axis],
								 tempVertices[tempIndices[tempInd + 2u]][current_scene_axis]));
...
CRT_Hit DSTree::traceTriangle(LiteMath::float3 Position, LiteMath::float3 Direction, LiteMath::float4* tempVertices, unsigned* tempIndices) {
	...
	//const LiteMath::float3  P{ Direction.y * E2.z - Direction.z * E2.y, Direction.z * E2.x - Direction.x * E2.z, Direction.x * E2.y - Direction.y * E2.x };
	const float d = dot(E1, P);
	//const float d = E1.x * P.x + E1.y * P.y + E1.z * P.z;
	...
	if (d < -LiteMath::EPSILON || d > LiteMath::EPSILON) {
		...
		//const float temp_u = f * (T.x * P.x + T.y * P.y + T.z * P.z);
		const LiteMath::float3 Q = cross(T, E1);
		//const LiteMath::float3 Q{ T.y * E1.z - T.z * E1.y, T.z * E1.x - T.x * E1.z, T.x * E1.y - T.y * E1.x };
		const float temp_v = f * dot(Direction, Q);
		//const float temp_v = f * (Direction.x * Q.x + Direction.y * Q.y + Direction.z * Q.z);
		const float temp_t = f * dot(E2, Q);
		//const float temp_t = f * (E2.x * Q.x + E2.y * Q.y + E2.z * Q.z);
...
CRT_Hit DSTree::RayQuery_NearestHit(LiteMath::float4 posAndNear, LiteMath::float4 dirAndFar) {
	...
	//for (uint32_t i = 1u; i < instances_info.size(); ++i)
	//	insts.push_back(i);
...
CRT_Hit DSTree::findHit(LiteMath::float4 posAndNear, LiteMath::float4 dirAndFar, bool findAny, uint32_t current_instance) {
	...
	LiteMath::float2 tMinMax(0.f, MAXFLOAT);
	...
	//std::vector <LiteMath::float2> tMinMaxes = { tMinMax };
	...
	//unsigned treeDepthMin = 1u, treeDepthMax = 50000u; //root has depth == 1
	...
	unsigned node = 0u;
	//1.f / ((treeDepthMax - treeDepthMin + 1u) * (1u + ((tempMeshSize - tempMesh.DSTreeOffset) >> treeDepthMax)))
	bool isFin[3] = { std::isfinite(invDir.x), std::isfinite(invDir.y), std::isfinite(invDir.z) };
		...
		do {
			...
			if (!CHECK_LEAF_POLY || IS_CARVING_NODE) {
				...
				if (!planeTrav[0] && (IS_CARVING_NODE || !CHECK_LEAF_POLY)) {
					...
					if (isFin[planes.y]) t[1] = invDir[planes.y] * (curr_node.planes[1u] - position[planes.y]);
					/*
					if (rightNodes.size() >= treeDepthMin && rightNodes.size() <= treeDepthMax && planeTrav[0]) {
						temp_hit.coords[3] += weight;
						temp_hit.coords[2] += weight;
					}*/
/*
					unsigned far = t[0] <= t[1], near = t[0] > t[1];
					if (!IS_CARVING_NODE) {
						planeTrav[far] = t[far] <= tMinMax.y;
						planeTrav[near] = t[near] >= tMinMax.x;
					}
					if (IS_CARVING_NODE && !IS_DOUBLE_CARVE) {
						float tMin[2] = { tMinMax.x, t[near] }, tMax[2] = { t[far], tMinMax.y };
						planeTrav[0] = tMin[tMin[0] < tMin[1]] <= tMax[tMax[0] > tMax[1]];
						planeTrav[1] = false;
					}
					if (IS_DOUBLE_CARVE) {
						float t2[2] = { tMinMax.x, tMinMax.y };
						bool tempTrav = false;
						if (!(planeTrav[0] || planeTrav[1])) tempTrav = t[t[0] < t[1]] < tMinMax.y;
						if (planeTrav[0] && !planeTrav[1]) {
							t2[0] = t[0];
							t[0] = tMinMax.x;
							tempTrav = t[t[0] < t[1]] <= t2[t2[0] > t2[1]];
						}
						if (!planeTrav[0] && planeTrav[1]) {
							t2[0] = t[1];
							t[1] = tMinMax.x;
						...
						if (planeTrav[0] && planeTrav[1]) tempTrav = tMinMax.x <= t[t[0] > t[1]];
						planeTrav[1] = false;
					}
					*/
					if (isFin[planes.x] && isFin[planes.y]) {
						...
						if (!IS_CARVING_NODE) {
							...
							/*if (rightNodes.size() >= treeDepthMin && rightNodes.size() <= treeDepthMax) {
								if (planeTrav[0]) temp_hit.coords[3] += weight;
								if (planeTrav[1]) temp_hit.coords[2] += weight;
							}*/
						}
		...
		if (CHECK_LEAF_POLY) {
			...
			for (unsigned j = curr_node.leftChild; j < tr_num; ++j) {
				...
				//temp_hit.coords[2] += 27 * weight;
				//temp_hit.coords[3] += 27 * weight;
