...
void Integrator_Generated::UpdateVectorMembers(std::shared_ptr<vk_utils::ICopyEngine> a_pCopyEngine)
{
  ...
  if(m_materials.size() > 0)
    a_pCopyEngine->UpdateBuffer(m_vdata.m_materialsBuffer, 0, m_materials.data(), m_materials.size()*sizeof(struct GLTFMaterial) );
...
{ 
  for(int i=0;i<m_vdata.m_texturesArrayTexture.size();i++)
    a_pCopyEngine->UpdateImage(m_vdata.m_texturesArrayTexture[i], m_textures[i]->getRawData(), m_textures[i]->width(), m_textures[i]->height(), m_textures[i]->bpp(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL); 
...
void Integrator_Generated::PathTraceMegaCmd(uint tid, uint a_maxDepth, float4* out_color)
{
...
void Integrator_Generated::NaivePathTraceMegaCmd(uint tid, uint a_maxDepth, float4* out_color)
{
  uint32_t blockSizeX = 256;
  uint32_t blockSizeY = 1;
  uint32_t blockSizeZ = 1;
...
void Integrator_Generated::PathTraceCmd(VkCommandBuffer a_commandBuffer, uint tid, uint a_maxDepth, float4* out_color)
{
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT }; 
  vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, PathTraceMegaLayout, 0, 1, &m_allGeneratedDS[0], 0, nullptr);
  PathTraceMegaCmd(tid, a_maxDepth, out_color);
  vkCmdPipelineBarrier(m_currCmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &memoryBarrier, 0, nullptr, 0, nullptr); 
}

void Integrator_Generated::NaivePathTraceCmd(VkCommandBuffer a_commandBuffer, uint tid, uint a_maxDepth, float4* out_color)
{
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT }; 
  vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, NaivePathTraceMegaLayout, 0, 1, &m_allGeneratedDS[1], 0, nullptr);
  NaivePathTraceMegaCmd(tid, a_maxDepth, out_color);
  vkCmdPipelineBarrier(m_currCmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &memoryBarrier, 0, nullptr, 0, nullptr); 
}
...
void Integrator_Generated::CastSingleRayCmd(VkCommandBuffer a_commandBuffer, uint tid, uint* out_color)
{
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT }; 
  vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, CastSingleRayMegaLayout, 0, 1, &m_allGeneratedDS[2], 0, nullptr);
...
void Integrator_Generated::PackXYCmd(VkCommandBuffer a_commandBuffer, uint tidX, uint tidY)
{
  m_currCmdBuffer = a_commandBuffer;
  VkMemoryBarrier memoryBarrier = { VK_STRUCTURE_TYPE_MEMORY_BARRIER, nullptr, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT }; 
  vkCmdBindDescriptorSets(a_commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, PackXYMegaLayout, 0, 1, &m_allGeneratedDS[3], 0, nullptr);
...
void Integrator_Generated::PathTraceBlock(uint tid, uint a_maxDepth, float4* out_color, uint32_t a_numPasses)
{
  ...
  VkBuffer out_colorGPU = vk_utils::createBuffer(device, tid*sizeof(struct LiteMath::float4 ), outFlags);
  ...
  //
  {
    ...
    beginCommandBufferInfo.flags = VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT;
    vkBeginCommandBuffer(commandBuffer, &beginCommandBufferInfo);
    PathTraceCmd(commandBuffer, tid, a_maxDepth, out_color);      
    vkEndCommandBuffer(commandBuffer);  
    auto start = std::chrono::high_resolution_clock::now();
  ...
  pCopyHelper->ReadBuffer(out_colorGPU, 0, out_color, tid*sizeof(struct LiteMath::float4 ));
...
void Integrator_Generated::NaivePathTraceBlock(uint tid, uint a_maxDepth, float4* out_color, uint32_t a_numPasses)
{
  // (1) get global Vulkan context objects
  //
  VkInstance       instance       = m_ctx.instance;
  ...
  //
  {
    ...
    beginCommandBufferInfo.flags = VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT;
    vkBeginCommandBuffer(commandBuffer, &beginCommandBufferInfo);
    NaivePathTraceCmd(commandBuffer, tid, a_maxDepth, out_color);      
    vkEndCommandBuffer(commandBuffer);  
    auto start = std::chrono::high_resolution_clock::now();
    for(uint32_t pass = 0; pass < a_numPasses; pass++)
      vk_utils::executeCommandBufferNow(commandBuffer, computeQueue, device);
...
void Integrator_Generated::CastSingleRayBlock(uint tid, uint* out_color, uint32_t a_numPasses)
{
  ...
  std::vector<vk_utils::VulkanImageMem*> images;
  auto beforeCreateObjects = std::chrono::high_resolution_clock::now();
  VkBuffer out_colorGPU = vk_utils::createBuffer(device, tid*sizeof(LiteMath::uint ), outFlags);
  buffers.push_back(out_colorGPU);
  ...
  //
  auto beforeCopy2 = std::chrono::high_resolution_clock::now();
  pCopyHelper->ReadBuffer(out_colorGPU, 0, out_color, tid*sizeof(LiteMath::uint ));
  this->ReadPlainMembers(pCopyHelper);
  afterCopy2 = std::chrono::high_resolution_clock::now();
